# Launcher for the CI-built portable bundle (MRC-windows-x64.zip).
# Uses the bundled Python (backend\python-x64) and the built frontend
# (frontend\dist + dist-electron) — no dev server, no venv setup needed.
param(
...
$pythonExe = Join-Path $repoRoot "backend\python-x64\python.exe"
$electronExe = Join-Path $repoRoot "frontend\node_modules\electron\dist\electron.exe"
$ffmpegExe = Join-Path $repoRoot "vendor\ffmpeg\windows\bin\ffmpeg.exe"

if (!(Test-Path $pythonExe)) {
  throw "Bundled Python was not found ($pythonExe). This script is for the CI-built bundle; for a source checkout use scripts\run_windows.ps1."
}
...
Write-Host "Starting MRC Integrated Acquisition (portable x64 bundle)" -ForegroundColor Cyan
